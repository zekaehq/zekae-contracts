...
contract XcmOracle is Ownable {
    ...
    function setTokenAmount(bytes2 _currencyId, uint256 _assetAmount, uint256 _vAssetAmount) public onlyOwner {
    ...
    function getVTokenByToken(address _assetAddress, uint256 _assetAmount) public view returns (uint256) {
        ...
        require(poolInfo.vAssetAmount != 0 && poolInfo.assetAmount != 0, "Not ready");
        uint256 mintFee = (rateInfo.mintRate * _assetAmount) / 10000;
        uint256 assetAmountExcludingFee = _assetAmount - mintFee;
        uint256 vAssetAmount = (assetAmountExcludingFee * poolInfo.vAssetAmount) / poolInfo.assetAmount;
        return vAssetAmount;
    }
    ...
    function getTokenByVToken(address _assetAddress, uint256 _vAssetAmount) public view returns (uint256) {
        ...
        uint256 assetAmount = (vAssetAmountExcludingFee * poolInfo.assetAmount) / poolInfo.vAssetAmount;
        return assetAmount;
    }
    ...
    function getCurrencyIdByAssetAddress(address _assetAddress) public view returns (bytes2) {
    ...
    /// @param _assetAddress The asset address.
    /// @param _currencyId The currencyId.
    function setAddressToCurrencyId(address _assetAddress, bytes2 _currencyId) public onlyOwner {
